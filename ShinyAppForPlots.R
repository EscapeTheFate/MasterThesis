  ...
  sidebarLayout(
    sidebarPanel(width = 2,
                 ...
                 selectInput("y_scale_fix", "Fix Y-Scale:",
                             choices = c("None", "Fixed on T", "Variance comparison", "Fixed on Omega"),
                             selected = "None"),
...
server <- function(input, output) {
  ...
  output$error_plot <- renderPlot({
    ...
    if (input$y_scale_fix == "Variance comparison" && input$plot_type == "error") {
    ...
    if (input$y_scale_fix == "Variance comparison" && input$plot_type == "sd") {
